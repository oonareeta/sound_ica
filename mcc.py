...
# Pytorch implementation does not work!!!
    elif method == 'cos':
        ...
        if x.shape == y.shape:
            ...
            for j in range(x.shape[0]):
                for k in range(y.shape[1]):
                    arr[j, k] = cos(x[:,j], y[:,k]) # used for for columns
...
def max_perm_mcs_col(A, true_A, method='cos'):
    ...
    Expectus that A and true_A are matrices (2D arrays).
    """
    if type(A) != type(true_A):
        raise ValueError('inputs are of different types: ({}, {})'.format(type(x), type(y)))
    if isinstance(A, np.ndarray):
        mean_mcc = np.mean(np.max(get_mcc_np(A, true_A, method), axis=1))
    elif isinstance(A, torch.Tensor):
        mean_mcc = np.mean(np.max(np.asarray(get_mcc_pt(A, true_A, method)), axis=1))
    else:
        raise ValueError('not a supported input type: {}'.format(type(x)))
    return mean_mcc
